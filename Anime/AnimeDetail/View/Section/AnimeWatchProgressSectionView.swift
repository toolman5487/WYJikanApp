//
//  AnimeWatchProgressSectionView.swift
//  WYJikanApp
//

import SwiftUI

struct AnimeWatchProgressSectionView: View {
    ...
    private var savedProgressView: some View {
        VStack(alignment: .leading, spacing: 12) {
            if let progress = item.watchProgressFraction(totalEpisodes: totalEpisodes) {
                HStack(spacing: 8) {
    ...
    private func episodeStepperView(for item: MyListCollectionItem) -> some View {
        HStack(spacing: 8) {
            ...
            .opacity((item.currentEpisode ?? 0) == 0 ? 0.42 : 1)

            Button {
                onEdit(item)
            } label: {
                Text(episodeButtonText(for: item))
            ...
            .buttonStyle(.plain)
            .disabled(isAtLastEpisode(item))
            .opacity(isAtLastEpisode(item) ? 0.42 : 1)
        }
    }
